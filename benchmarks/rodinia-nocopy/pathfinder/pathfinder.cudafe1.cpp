...
{ 
# 123
__attribute__((unused)) static int prev[256]; 
# 124
__attribute__((unused)) static int result[256]; 
# 126
int bx = (blockIdx.x); 
# 127
int tx = (threadIdx.x); 
...
dst = temp; 
# 207
cudaConfigureCall(dimGrid, dimBlock) ? ((void)0) : dynproc_kernel((pyramid_height <= ((rows - t) - 1)) ? pyramid_height : ((rows - t) - 1), gpuWall, gpuResult[src], gpuResult[dst], cols, rows, t, borderCols); 
# 211
...
init(argc, argv); 
# 232
int borderCols = (pyramid_height * 1); 
# 233
int smallBlockCol = (256 - ((pyramid_height * 1) * 2)); 
# 234
int blockCols = ((cols / smallBlockCol) + (((cols % smallBlockCol) == 0) ? 0 : 1)); 
# 236
printf("pyramidHeight: %d\ngridSize: [%d]\nborder:[%d]\nblockSize: %d\nblockGrid:[%d]\ntargetBlock:[%d]\n", pyramid_height, cols, borderCols, 256, blockCols, smallBlockCol); 
# 239
...
m5_work_begin(0, 0); 
# 252
int final_ret = calc_path(data + cols, gpuResult, rows, cols, pyramid_height, blockCols, borderCols); 
# 255
m5_work_end(0, 0); 
# 259
...
#define _NV_ANON_NAMESPACE _GLOBAL__N__18_pathfinder_cpp4_ii_2e3c706b
# 1 "pathfinder.cudafe1.stub.c"
