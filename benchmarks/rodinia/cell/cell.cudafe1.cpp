...
J, int K, int L, int 
# 79
bornMin, int bornMax, int 
# 80
dieMin, int dieMax, int 
# 81
blockL, int border) ;
#if 0
# 82
{ 
# 83
__attribute__((unused)) static int data[8][8][8]; 
# 84
__attribute__((unused)) static int result[8][8][8]; 
# 86
int bj = (blockIdx.x); 
# 87
...
# 107
int blkJmax = ((blkJ + 8) - 1); 
# 108
int blkKmax = ((blkK + 8) - 1); 
# 109
int blkLmax = ((blkL + 8) - 1); 
# 112
int idxJ = (blkJ + tj); 
# 113
int idxK = (blkK + tk); 
...
# 118
if ((((idxJ >= 0) && (idxJ <= (J - 1))) && ((idxK >= 0) && (idxK <= (K - 1)))) && ((idxL >= 0) && (idxL <= (L - 1)))) 
# 120
{ 
# 121
...
# 128
int validJmax = ((blkJmax > (J - 1)) ? ((8 - 1) - ((blkJmax - J) + 1)) : (8 - 1)); 
# 129
int validKmin = ((blkK < 0) ? (-blkK) : 0); 
# 130
int validKmax = ((blkKmax > (K - 1)) ? ((8 - 1) - ((blkKmax - K) + 1)) : (8 - 1)); 
# 131
int validLmin = ((blkL < 0) ? (-blkL) : 0); 
# 132
int validLmax = ((blkLmax > (L - 1)) ? ((8 - 1) - ((blkLmax - L) + 1)) : (8 - 1)); 
# 134
int isValid = ((((tj >= validJmin) && (tj <= validJmax)) && ((tk >= validKmin) && (tk <= validKmax))) && ((tl >= validLmin) && (tl <= validLmax))); 
# 138
bool computed = false; 
# 139
...
computed = false; 
# 141
int inLayer = ((((tj >= (i + 1)) && (tj <= ((8 - i) - 2))) && ((tk >= (i + 1)) && (tk <= ((8 - i) - 2)))) && ((tl >= (i + 1)) && (tl <= ((8 - i) - 2)))); 
# 145
if ((isValid) && (inLayer)) { 
# 146
computed = true; 
...
a = j; b = k; c = l; 
# 154
a = ((a < validJmin) ? validJmin : ((a > validJmax) ? validJmax : a)); 
# 155
b = ((b < validKmin) ? validKmin : ((b > validKmax) ? validKmax : b)); 
# 156
c = ((c < validLmin) ? validLmin : ((c > validLmax) ? validLmax : c)); 
# 157
sum += ((((data)[a])[b])[c]); 
# 158
}  
...
# 163
((((result)[tj])[tk])[tl]) = 0; } else { 
# 164
if ((orig == 0) && ((sum >= bornMin) && (sum <= bornMax))) { 
# 165
((((result)[tj])[tk])[tl]) = 1; } else { 
# 167
...
#endif
# 188 "cell.cu"
int simulate(int *gpuData[2], int 
# 189
J, int K, int L, int 
# 190
timesteps, int 
...
pyramid_height, int 
# 192
blockJ, int blockK, int blockL, int 
# 193
border) 
# 194
{ 
...
dst = temp; 
# 204
cudaConfigureCall(dimGrid, dimBlock) ? ((void)0) : evolve((pyramid_height <= (timesteps - t)) ? pyramid_height : (timesteps - t), gpuData[src], gpuData[dst], J, K, L, bornMin, bornMax, dieMin, dieMax, blockL, border); 
# 210
}  
# 211
return dst; 
...
# 218
if (num_devices > 1) { cudaSetDevice(1); }  
# 220
init(argc, argv); 
# 222
run(argc, argv); 
...
init(argc, argv); 
# 236
int border = ((pyramid_height * 2) / 2); 
# 237
int smallBlockEdge = (8 - (pyramid_height * 2)); 
# 238
...
# 242
printf("pyramidHeight: %d\ngridSize: [%d, %d, %d]\nborder:[%d]\nblockSize: %d\nblockGrid:[%d, %d, %d]\ntargetBlock:[%d]\n", pyramid_height, J, K, L, border, 8, blockJ, blockK, blockL, smallBlockEdge); 
# 246
m5_work_begin(0, 0); 
# 249
int *gpuData[2]; 
...
# 256
int ret = simulate(gpuData, J, K, L, timesteps, pyramid_height, blockJ, blockK, blockL, border); 
# 260
...
#define _NV_ANON_NAMESPACE _GLOBAL__N__12_cell_cpp4_ii_bornMin
# 1 "cell.cudafe1.stub.c"
