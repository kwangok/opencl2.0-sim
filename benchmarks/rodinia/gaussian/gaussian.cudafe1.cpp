...
timeval time_start; 
# 90
gettimeofday(&time_start, __null); 
# 93
m5_work_begin(0, 0); 
# 97
ForwardSub(); 
...
timeval time_end; 
# 101
gettimeofday(&time_end, __null); 
# 102
unsigned time_total = ((((time_end.tv_sec) * (1000000)) + (time_end.tv_usec)) - (((time_start.tv_sec) * (1000000)) + (time_start.tv_usec))); 
# 104
BackSub(); 
# 107
m5_work_end(0, 0); 
...
# 114
printf("\nTime total (including memory transfers)\t%f sec\n", time_total * (9.999999999999999547e-07)); 
# 115
printf("Time for CUDA kernels:\t%f sec\n", totalKernelTime * (9.999999999999999547e-07)); 
# 120
free(m); 
# 121
free(a); 
...
for (t = 0; t < (Size - 1); t++) { 
# 292
cudaConfigureCall(dimGrid, dimBlock) ? ((void)0) : Fan1(m_cuda, a_cuda, Size, t); 
# 293
cudaThreadSynchronize(); 
# 294
cudaConfigureCall(dimGridXY, dimBlockXY) ? ((void)0) : Fan2(m_cuda, a_cuda, b_cuda, Size, Size - t, t); 
# 295
cudaThreadSynchronize(); 
# 296
checkCUDAError("Fan2"); 
...
#define _NV_ANON_NAMESPACE _GLOBAL__N__16_gaussian_cpp4_ii_bd85c056
# 1 "gaussian.cudafe1.stub.c"
