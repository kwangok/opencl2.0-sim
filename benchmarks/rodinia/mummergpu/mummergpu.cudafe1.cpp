...
nodes) 
# 419
{int volatile ___ = 1;(void)cur;(void)use_two_level;(void)nodes;
...
childrenarr) 
# 499
{int volatile ___ = 1;(void)cur;(void)use_two_level;(void)childrenarr;
...
int qry_match_len = 0; 
# 901
_MatchCoord *result = ((((_MatchCoord *)match_coords) + qryAddr) - __umul24(qryid, min_match_len + 1)); 
# 903
queries += qryAddr; 
# 905
int last = (qlen - min_match_len); 
# 906
for (int qrystart = 0; qrystart <= last; (qrystart++), (result += 1)) 
# 910
{ 
# 912
unsigned node_start; 
...
# 923
char c = (queries[qrystart + qry_match_len]); 
# 925
do { } while (0); 
# 928
int refpos = 0; 
...
# 934
(children.data) = getChildren(cur, false, childrenarr); 
# 935
prev = cur; 
# 936
uchar3 next; 
...
_PixelOfNode node; 
# 962
(node.data) = getNode(cur, true, nodes); 
# 963
node_start = ((((node.start).x) | (((node.start).y) << 8)) | (((node.start).z) << 16)); 
# 964
unsigned node_end = ((((node.end).x) | (((node.end).y) << 8)) | (((node.end).z) << 16)); 
# 966
do { } while (0); 
# 967
{ 
...
# 984
c = (queries[qrystart + qry_match_len]); 
# 986
while ((refpos <= node_end) && (c != ('\000'))) 
# 987
{ 
# 988
char r = getRef(refpos, ref); 
# 990
do { } while (0); 
# 992
if (r != c) 
...
{ 
# 1012
set_result(cur, result, refpos - node_start, qry_match_len, min_match_len, 0); 
# 1015
...
_PixelOfNode node; 
# 1021
(node.data) = getNode(prev, false, nodes); 
# 1022
...
do { } while (0); 
# 1258
char queryflankingbase = (queries[(matches[matchid]).qrystartpos]); 
# 1261
unsigned matchaddr = (((matches[matchid]).matchnode).data); 
# 1262
unsigned cur = matchaddr; 
# 1263
unsigned printParent = cur; 
# 1265
...
do { } while (0); 
# 1270
while (((((node.depth).x) | (((node.depth).y) << 8)) | (((node.depth).z) << 16)) > min_match_length) 
# 1271
{ 
# 1272
printParent = cur; 
...
# 1291
int matchlen = (((((node.depth).x) | (((node.depth).y) << 8)) | (((node.depth).z) << 16)) - 1); 
# 1292
...
# 1302
matchlen = ((((((node.depth).x) | (((node.depth).y) << 8)) | (((node.depth).z) << 16)) - 1) + ((matches[matchid]).edgematch)); 
# 1303
}  
# 1305
depthToGoldenPath = 1; 
...
{ 
# 1324
int leafid = ((((children123.leafid).x) | (((children123.leafid).y) << 8)) | (((children123.leafid).z) << 16)); 
# 1325
...
# 1364
if (pa == cur) { curchild = 'C'; } else { 
# 1365
if (pc == cur) { curchild = 'G'; } else { 
# 1366
if (pg == cur) { curchild = 'T'; } else { 
# 1367
if (pt == cur) { curchild = '$'; } else 
# 1369
{ 
# 1370
forceToParent = true; 
...
node = getNodeNoData(cur, true, nodes); 
# 1380
matchlen = (((((node.depth).x) | (((node.depth).y) << 8)) | (((node.depth).z) << 16)) - 1); 
# 1381
}  
# 1382
} else 
...
{ 
# 1395
goldenChild = (queries[(((matches[matchid]).qrystartpos) + matchlen) + 1]); 
# 1396
}  
# 1398
do 
...
#define _NV_ANON_NAMESPACE _GLOBAL__N__17_mummergpu_cpp4_ii_a6baf3c4
# 1 "mummergpu.cudafe1.stub.c"
