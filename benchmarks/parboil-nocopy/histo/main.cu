...
//	cudaMalloc((void**)&final_histo     , img_width*histo_height*sizeof(unsigned char));
	...
	sm_mappings                         = (uchar4*)malloc(img_width*img_height*sizeof(uchar4));
	global_subhisto                     = (unsigned int *)malloc(img_width*histo_height*sizeof(unsigned int));
	global_histo                        = (unsigned short *)malloc(img_width*histo_height*sizeof(unsigned short));
	global_overflow                     = (unsigned int *)malloc(img_width*histo_height*sizeof(unsigned int));
	final_histo                         = (unsigned char*)malloc(img_width*histo_height*sizeof(unsigned char));

	assert(sm_mappings);
	assert(global_subhisto);
	assert(global_histo);
	assert(global_overflow);
	assert(final_histo);
...
//		cudaMemcpy(ranges,ranges_h, 2*sizeof(unsigned int), cudaMemcpyHostToDevice);
		...
		histo_prescan_kernel<<<dim3(PRESCAN_BLOCKS_X),dim3(PRESCAN_THREADS)>>>((unsigned int*)img, img_height*img_width, ranges_h);
...
//	cudaFree(final_histo);
	...
	printf("line 212: pb_SwitchToTimer done\n");

	free(img);
	free(histo);

	pb_SwitchToTimer(&timers, pb_TimerID_NONE);
	printf("line 218: pb_SwitchToTimer done\n");

	printf("\n");
	pb_PrintTimerSet(&timers);
	printf("line 223: pb_PrintTimerSet done\n");
