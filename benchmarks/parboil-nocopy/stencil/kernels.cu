...
__global__ void block2D_hybrid_coarsen_x(float c0,float c1,float *A0,float *Anext, int nx, int ny, int nz)
{
	...
	for(int k=1;k<nz-1;k++)
	{
		...
		if(w_region) {
			...
			Anext[Index3D (nx, ny, i, j, k)] = (a_left_right + sh_A0[sh_id+1] + a_top + a_down + top + bottom)*c1 - sh_A0[sh_id]*c0 ;	
		...
		if(w_region2) {
			...
			Anext[Index3D (nx, ny, i2, j, k)] = (a_left_right + sh_A0[sh_id2-1] + a_top + a_down + top2 + bottom2)*c1 - sh_A0[sh_id2]*c0;
		}
